...
#include <stdlib.h>
#include <stdio.h>
#define HEAP_SIZE 2000 	//how many total memory spots in linked list?
#define ALLOCATE 11		//how many objects in heap?
#define NUM_ROOTS 2		//how many roots?
...
void parse_structure(int addr){
    find_node(addr);
    if (curr->type == "IND" || curr->type == "SOFT" || curr->type == "WEAK" || curr->type == "VAR"){
		find_node(curr->val1);
		if (curr->color == "white"){
			find_node(addr);
        	move_to_grey(curr->val1);
		}
	}else if ((curr->type == "TUP") || (curr->type == "CALL")){
		find_node(curr->val1);
		if (curr->color == "white"){
			find_node(addr);
        	move_to_grey(curr->val1);
...
void reset_heap(){
	...
	int i;
	for (i=0; i<HEAP_SIZE; i++){
		if (curr->color == "black" || curr->color == "grey"){
			curr->color = "white";
		}
		curr = curr->next;
	}
...
int has_references(int addr){
	...
	for (i=0; i<HEAP_SIZE-1; i++){
		if((curr->type=="IND")||(curr->type=="WEAK")||(curr->type=="SOFT")||(curr->type=="VAR")){
...
int main(int argc, char** argv){
	...
	printf("-----------------------------------------\n");
	int nr2[] = {27};
	int nv12[] = {1,18,3,0,0,12,0,3,13,36};
	int nv22[] = {0,0,12,0,0,12,0,15,0,12};
	char *nt2[] = {"BOOL", "IND", "TUP","IND", "IND", "TUP","NULL", "CALL", "INT", "CALL"};
